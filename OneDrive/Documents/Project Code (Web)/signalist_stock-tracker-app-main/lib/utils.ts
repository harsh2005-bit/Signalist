...
/**
 * Normalizes stock symbols for TradingView widgets
 * Handles Indian stock formats (.NS, .BO) and ensures proper exchange prefixes
 * @param symbol - Stock symbol in any format (e.g., "RELIANCE.NS", "NSE:RELIANCE", "AAPL")
 * @returns Normalized symbol for TradingView (e.g., "NSE:RELIANCE", "NASDAQ:AAPL")
 */
export const normalizeSymbolForTradingView = (symbol: string): string => {
  if (!symbol) return symbol;
  
  const upperSymbol = symbol.toUpperCase().trim();
  
  // If already in TradingView format (NSE:XXX or BSE:XXX), return as is
  if (upperSymbol.startsWith('NSE:') || upperSymbol.startsWith('BSE:')) {
    return upperSymbol;
  }
  
  // Handle Indian stock formats (.NS for NSE, .BO for BSE)
  if (upperSymbol.endsWith('.NS')) {
    return `NSE:${upperSymbol.replace('.NS', '')}`;
  }
  ...
  // For US stocks, check if we need to add exchange prefix
  // Common US exchanges: NASDAQ, NYSE
  // For now, return uppercase as-is (TradingView can handle most US symbols without prefix)
  // But if it's a known Indian stock symbol, add NSE prefix
  const indianStocks = ['RELIANCE', 'TCS', 'INFY', 'HDFCBANK', 'ICICIBANK', 'LT', 'HDFC', 'SBIN', 'BHARTIARTL', 'ITC'];
  if (indianStocks.includes(upperSymbol)) {
    return `NSE:${upperSymbol}`;
  }
  
  // Return uppercase for other symbols
  return upperSymbol;
};
