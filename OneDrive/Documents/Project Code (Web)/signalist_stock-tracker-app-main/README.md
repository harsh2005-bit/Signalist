...
2. All your API keys and credentials ready
...
- **MongoDB Atlas**: Make sure your MongoDB Atlas IP whitelist includes Vercel's IP ranges or allows all IPs (`0.0.0.0/0`) for testing
- **BETTER_AUTH_URL**: Update this to your Vercel deployment URL after first deployment
- **Inngest**: For production, configure Inngest with your Vercel deployment URL
- **Build**: The project uses standard Next.js build (Turbopack is only for local development)
...
- **API Routes**: Ensure all API routes are properly configured in `app/api/` directory
...
## 🔄 Syncing TradingView Symbols

To sync the latest stock symbols from TradingView for supported exchanges (e.g., NSE, BSE), run the following command:

```bash
npm run sync:symbols
```

This will fetch symbols and save them to `data/tradingview-symbols.json`. Ensure you have internet access and the endpoint is reachable. Note that this script uses an unofficial TradingView API and may be subject to rate limits or changes.
